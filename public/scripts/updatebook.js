...
deleteButtons.forEach((x)=>{
    x.addEventListener('click',async ()=>{
        fetch('https://msns-soi.vercel.app/getbook/'+x.classList[1]).then((res)=>{return res.json()}).then((book)=>{
        ...
        await fetch('https://msns-soi.vercel.app/deletebook/'+x.classList[1]);
...
const openUpdateBookPopUpOverlay=(book,bookid)=>{
    ...
    document.querySelector('#admin-addbook-update-button').addEventListener('click',()=>{
        ...
        fetch('https://msns-soi.vercel.app/admin/updatebook',{body:JSON.stringify(newbook),method:"POST",headers: { 'Content-Type': 'application/json' }}).then((res)=>{return res.json()})
    .then((response)=>{
        if(response.status=='fail'){
            ...
            /*location.reload().then(()=>{
                ...
                setTimeout(()=>{
                    //window.location.href='https://msns-soi.vercel.app/admin/search/#search-bookDisplay'
                },200);
