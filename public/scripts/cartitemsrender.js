...
async function fetchCart() {
  try {
    const response = await fetch('https://msns-soi.vercel.app/cart/'+currentusername.textContent); // Replace with your actual API endpoint
...
async function createBookCellNew(bookid) {
    fetch('https://msns-soi.vercel.app/getbook/'+`${bookid}`).then((res)=>{return res.json()}).then((bookData)=>{
        fetch('https://msns-soi.vercel.app/cart/'+`${currentusername.textContent}`,{body:JSON.stringify({id:bookid}),method:"POST",headers: { 'Content-Type': 'application/json' }}).then(()=>{
            ...
            deleteButton.addEventListener('click', async function() {
                ...
                cartItemCounter.textContent=Number(cartItemCounter.textContent)-1;
                await fetch('https://msns-soi.vercel.app/cart/delete/'+`${currentusername.textContent}`,{body:JSON.stringify({id:bookid}),method:"POST",headers: { 'Content-Type': 'application/json' }})
