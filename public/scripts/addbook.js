...
submitButton.addEventListener('click',()=>{
    ...
    fetch('https://msns-soi.vercel.app/admin/addbook',{body:JSON.stringify(newBook),method:"POST",headers: { 'Content-Type': 'application/json' }}).then((res)=>{return res.json()})
    .then((response)=>{
        if(response.status=='fail'){
            ...
            setTimeout(
                ()=>{
                    window.location.href="https://msns-soi.vercel.app/admin"
                },3100
            );
