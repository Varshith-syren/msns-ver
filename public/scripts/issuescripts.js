...
async function createBookEntry(user) {
  ...
  for(let x of user.bookid){  
    await fetch('https://msns-soi.vercel.app/getbook/'+`${x}`).then((res)=>{return res.json()}).then((book)=>{
      ...
      issueButton.addEventListener('click',async ()=>{
        await fetch('https://msns-soi.vercel.app/borrow/'+`${user.username}`,{body:JSON.stringify({books:[x]}),method:"POST",headers: { 'Content-Type': 'application/json' }});
        await fetch('https://msns-soi.vercel.app/issue/delete/'+`${user.username}`,{body:JSON.stringify({id:x}),method:"POST",headers: { 'Content-Type': 'application/json' }});
...
fetch('https://msns-soi.vercel.app/issue').then((res)=>{return res.json()}).then( (userList)=>{
  if (userList) {
