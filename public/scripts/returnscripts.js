...
async function createBookEntry(user) {
  ...
  for(let x of user.borrowed){  
    await fetch('https://msns-soi.vercel.app/getbook/'+`${x.bookid}`).then((res)=>{return res.json()}).then((book)=>{
      ...
      issueButton.addEventListener('click',async ()=>{
        await fetch('https://msns-soi.vercel.app/return/delete/'+`${user.username}`,{body:JSON.stringify({id:x.bookid}),method:"POST",headers: { 'Content-Type': 'application/json' }});
...
fetch('https://msns-soi.vercel.app/return').then((res)=>{return res.json()}).then( (userList)=>{
  if (userList) {
