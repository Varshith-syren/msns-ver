...
submitButton.addEventListener('click',()=>{
    ...
    fetch('https://msns-soi.vercel.app/search/user',{body:JSON.stringify(request),method:"POST",headers: { 'Content-Type': 'application/json' }}).then((res)=>{return res.json()}).then((data)=>{
        data.forEach(student => {
            if(student.username!='admin'){
                    ...
                    fetch('https://msns-soi.vercel.app/borrow/'+student.username).then((res)=>{return res.json()}).then((userBorrow)=>{
                        if (userBorrow.borrowed && userBorrow.borrowed.length > 0) {
                            userBorrow.borrowed.forEach(x => {
                                fetch('https://msns-soi.vercel.app/getbook/'+x.bookid).then((res)=>{return res.json()}).then((book)=>{
